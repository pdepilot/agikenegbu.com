...
(function () {
    ...
    const LIVESTREAM_CONFIG = {
        ...
        currentSession: {
            ...
            minister: 'Pastor Emmanuel Adeyemi',
            ministry: 'Global Worship Leader',
            country: 'Nigeria',
            worshipTeam: 'SDTG Mass Choir',
            photo: 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=600&q=80',
            topic: 'Atmosphere of Glory'
        },
        upcomingSession: {
            title: 'Keynote — When Glory Descends',
            minister: 'Rev. Dr. David Okafor',
            time: '8:00 PM WAT'
        },
        schedule: [
            { time: '6:00 PM', session: 'Pre-Service Worship', speaker: 'SDTG Worship Team', worship: 'Mass Choir' },
            { time: '7:00 PM', session: 'Opening Address', speaker: 'Rev. Pastor John Ikenegbu', worship: '—' },
            { time: '7:30 PM', session: 'Worship Encounter', speaker: 'Pastor Emmanuel Adeyemi', worship: 'Frank Edwards' },
            { time: '8:00 PM', session: 'Keynote Message', speaker: 'Rev. Dr. David Okafor', worship: '—' },
            { time: '9:30 PM', session: 'Altar Call & Prayer', speaker: 'Revival Team', worship: 'Ada Ehi' },
            { time: '10:30 PM', session: 'Closing Worship', speaker: '—', worship: 'Nathaniel Bassey' }
        ],
        pastBroadcasts: [
    ...
    function spawnConfetti(container) {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            p.style.background = ['#D4AF37', '#F5D061', '#4a2d7a', '#ffffff'][Math.floor(Math.random() * 4)];
    ...
    function initShare() {
        document.getElementById('lsInviteBtn')?.addEventListener('click', () => {
            const url = window.location.href;
            if (navigator.share) {
                navigator.share({ title: 'Watch SDTG Live', text: 'Join me for Send Down Thy Glory live!', url });
            } else {
                navigator.clipboard?.writeText(url);
                const btn = document.getElementById('lsInviteBtn');
                if (btn) { btn.innerHTML = '<i class="fas fa-check"></i> Link Copied'; setTimeout(() => { btn.innerHTML = '<i class="fas fa-user-plus"></i> Invite Friends'; }, 2500); }
            }
        });
    }
